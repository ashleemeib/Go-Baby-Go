...
void readData() {
  ...
  while (loopVar > 0) {
    ...
    total = total + ((myData.xData * 9.80665) - zO);
    loopVar--;
  }
  total = total / 5;
}
...
void loop()
{
  ...
  /* // detection from idle
    if (curVel >= zO && detM == false) {//absolute value to account for positive and negative change
     Serial.print("V:");
     ...
     detM = true;
    } else if ((curVel) < zO && detM == true) {//sto detection, parameters need testing
     Serial.print(" V: ");
     Serial.print(curVel, 4);
     Serial.println();
     Serial.print(" A: ");
