...
void loop()
{
  ...
  // detection from idle
  if ((abs(myData.zData * 9.80665)) > last && detb == false) {
    Serial.print(" a0: ");
    Serial.print((abs(myData.zData * 9.80665)), 4);
    Serial.print(" Z0: ");
    ...
    det1 = 0;//reset
    detb = true;
    last = ((myData.zData * 9.80665) - mT);

    //checking over 50 ms
  } else if (abs((myData.zData * 9.80665)) > last  && detb == true && dets == false) {
      Serial.print(" a0: ");
    Serial.print((abs(myData.zData * 9.80665)), 4);
      Serial.print(" Z0: ");
      Serial.print(abs((myData.zData * 9.80665)), 4);
  ...
  //first detects stop
  else if ((abs(myData.zData * 9.80665)) <= last && det < sampRate && detb == true && dets == true) {
    Serial.print(" a1: ");
    Serial.print((abs(myData.zData * 9.80665)), 4);
    Serial.print(" Z1: ");
    Serial.print(((myData.zData * 9.80665) - mT), 4);
    Serial.println();
    if ( last + last / 4 > (myData.zData * 9.80665) - mT) {
      det = 0;
      dets = false;
    } else {
      det += 1;
    ...
    last = ((myData.zData * 9.80665) - mT);
  } else if ((abs(myData.zData * 9.80665)) <= last && det == sampRate && detb == true) {
    Serial.print(" a0: ");
    Serial.print((abs(myData.zData * 9.80665)), 4);
    Serial.print(" Z2: ");
    Serial.print(((myData.zData * 9.80665) - mT), 4);
