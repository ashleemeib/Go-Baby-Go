...
void loop()
{
  ...
  if ( kxAccel.dataReady() )
  {
    ...
    while (timet != 0) {
      ...
      mTx = abs(myData.xData * 9.80665) + mTx;
      mTy = abs(myData.yData * 9.80665) + mTy;
      mTz = abs(myData.zData * 9.80665) + mTz;
      mT = (mTz) / 100;
      timet --;
    }
    // detection from idle
    if (abs((myData.zData * 9.80665) - mT ) > mT && detb == false) {
      Serial.print(" Z0: ");
    ...
    //first detects stop
    else if (abs((myData.zData * 9.80665) - mT) < mT && det < sampRate && detb == true) {
      Serial.print(" Z0: ");
      Serial.print(abs((myData.zData * 9.80665)*mT / 400 - 0.85), 4);
      Serial.println();
      det += 1;
      ...
      Serial.print(" Z0: ");
      Serial.print(abs((myData.zData * 9.8) - mT), 4);
      Serial.println();
    }
  }
